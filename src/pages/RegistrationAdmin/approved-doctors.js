...
import { getApprovedDoctors } from "store/registration-admin/actions";
import "assets/scss/table.scss";

class ApprovedDoctors extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    const { approvedDoctors, onGetApprovedDoctors } = this.props;
    onGetApprovedDoctors(this.state.user_id);
    this.setState({ approvedDoctors });
  }
  ...
  render() {
    ...
    const { approvedDoctors } = this.props;
    const { onGetApprovedDoctors } = this.props;

    const pageOptions = {
      sizePerPage: 10,
      totalSize: approvedDoctors.length, // replace later with size(approvedDoctors),
      custom: true,
    };
    ...
    return (
      <React.Fragment>
        <div className="page-content">
          <MetaTags>
            <title>Approved Doctors | Lab Hazir</title>
          </MetaTags>
          <Container fluid>
            {/* Render Breadcrumbs */}
            <Breadcrumbs title="Doctors" breadcrumbItem="Approved" />
            <Row>
              <Col lg="12">
                <Card>
                  <CardBody>
...
ApprovedDoctors.propTypes = {
  match: PropTypes.object,
  approvedDoctors: PropTypes.array,
  className: PropTypes.any,
  onGetApprovedDoctors: PropTypes.func,
};
const mapStateToProps = ({ registrationAdmin }) => ({
  approvedDoctors: registrationAdmin.approvedDoctors  || [],
});

const mapDispatchToProps = (dispatch, ownProps) => ({
  onGetApprovedDoctors: () => dispatch(getApprovedDoctors()),
});

export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withRouter(ApprovedDoctors));
